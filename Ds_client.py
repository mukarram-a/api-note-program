...
"send()" function called from 'a4_refactor.py' 
...
def send(server:str, port:int, username:str, password:str, message:str, bio:str=None):
  ...
  if bio is not None:
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client:   #establishes the connection between the program and the server
      ...
      send.write(join(username, password) + '\r\n')                     #send username and password to the server
      send.flush()
    ...
    if message == '':
      print('Username can not be empty. Returning...')                  #checks if the username parameter is blank and returns False and an error message to the user
      return False
